    ...
    const updateStatus = async () => {
        ...
        try {
            if (attachment) {
                ...
                const fileNameWithFolder = `status/${timestamp}-${attachment.name}`;

                const { data } = await axios.post(
                    "http://localhost:3000/api/jobs/signed-url",
                ...
                const filePathParts = data.fileUrl.split("/");
                fileUrl = filePathParts[filePathParts.length - 1];
            }

            const response = await axios.post(
                `http://localhost:3000/api/jobs/${jobId}/update-status`,
